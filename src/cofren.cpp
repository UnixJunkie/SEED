   ...
   CoEnEv_gr  evaluation of the coulombic interaction energy on a grid */
{
  ...
  double Weig_t,Weig_u,Weig_v,CoEnEv_gr, hlp1, hlp2, hlp3;

  CoEnEv_gr=0.0;

  for (i=1;i<=FrAtNu;i++) {
    // see also vwfren.cpp
    hlp1 = (RoSFCo[i][1] - (BSMinC[1] - CoGrIn)) / CoGrSi;
    hlp2 = (RoSFCo[i][2] - (BSMinC[2] - CoGrIn)) / CoGrSi;
    hlp3 = (RoSFCo[i][3] - (BSMinC[3] - CoGrIn)) / CoGrSi;
    GrCo_i = ffloor(hlp1) + 1; //ffloor((RoSFCo[i][1]-(BSMinC[1]-CoGrIn))/CoGrSi)+1;
    GrCo_j = ffloor(hlp2) + 1; //ffloor((RoSFCo[i][2]-(BSMinC[2]-CoGrIn))/CoGrSi)+1;
    GrCo_k = ffloor(hlp3) + 1; //ffloor((RoSFCo[i][3]-(BSMinC[3]-CoGrIn))/CoGrSi)+1;

    if ((GrCo_i>=1)&&(GrCo_i<CoGPoN[1])&&(GrCo_j>=1)&&(GrCo_j<CoGPoN[2])&&
        (GrCo_k>=1)&&(GrCo_k<CoGPoN[3])) {

      Weig_t = hlp1 - double(GrCo_i - 1); //(RoSFCo[i][1]-(BSMinC[1]-CoGrIn+(GrCo_i-1)*CoGrSi))/CoGrSi;
      Weig_u = hlp2 - double(GrCo_j - 1); //(RoSFCo[i][2]-(BSMinC[2]-CoGrIn+(GrCo_j-1)*CoGrSi))/CoGrSi;
      Weig_v = hlp3 - double(GrCo_k - 1); //(RoSFCo[i][3]-(BSMinC[3]-CoGrIn+(GrCo_k-1)*CoGrSi))/CoGrSi;
